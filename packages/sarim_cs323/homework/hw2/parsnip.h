...
enum {
   ...
   // Token types used by tokenize() and parse()
      ...
      SIMPLE,           // Maximal contiguous sequence ... (as above) - 0

      RED_IN,           // <    Redirect stdin to file - 1
      RED_IN_HERE,      // <<   Redirect stdin to HERE document - 2

      RED_OUT,          // >    Redirect stdout to file - 3
      RED_OUT_APP,      // >>   Append stdout to file - 4

      RED_ERR_OUT,      // 2>&1 Redirect stderr to stdout - 5
      RED_ERR_CLS,      // 2>&- Close stderr - 6
   ...
   // Types used only by parse()
      ...
      NONE,             // End of token list - 14
      ERROR,            // Error - 15
      SUBCMD,           // Subcommand - 16
      LOCAL,            // Local variable - 17
      ARG,              // Argument - 18
};
