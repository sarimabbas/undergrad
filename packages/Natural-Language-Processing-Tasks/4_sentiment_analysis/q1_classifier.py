...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(shape=(self.config.batch_size, self.config.n_features), dtype=tf.float32)
    self.labels_placeholder = tf.placeholder(shape=(self.config.batch_size, self.config.n_classes), dtype=tf.int32)
    ### END YOUR CODE
  ...
  def add_training_op(self, loss):
    ...
    train_op = optimizer.minimize(loss, var_list=tf.all_variables())
    ### END YOUR CODE
    return train_op
  ...
  def add_model(self, input_data):
    ...
    Args:
      input_data: A tensor of shape (batch_size, n_features). / it is the x in the equation
    Returns:
      out: A tensor of shape (batch_size, n_classes)
    """
    ### YOUR CODE HERE
    with tf.variable_scope("softmax"):
      W = tf.Variable(tf.zeros(shape=(self.config.n_features, self.config.n_classes), dtype=tf.float32), name="W")
      mul = tf.matmul(input_data, W) # (64 x 100) * (100 x 5) = (64 x 5)
      b = tf.Variable(tf.zeros(shape=(self.config.n_classes, ), dtype=tf.float32), name="b")
      out = softmax(mul + b) # the softmax function from q1_softmax
    ### END YOUR CODE
    return out
  ...
  def add_loss_op(self, pred):
    ...
    """
    ### YOUR CODE HERE
    loss = cross_entropy_loss(self.labels_placeholder, pred) # the one-hot is labels_placeholder. pred is prediction
    ### END YOUR CODE
    return loss
