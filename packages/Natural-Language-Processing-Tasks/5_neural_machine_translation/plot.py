...
def diagramMaker(output_words, attentions, input_sentence, savepath):
    figure = plt.figure()
    #print("created figure")
    sub = figure.add_subplot(111)
    #print("added subplot")
    matshow = sub.matshow(attentions.numpy(), cmap="bone") # Bone was on Piazza 544
    #print("added attentions")
    figure.colorbar(matshow)
    #print("added colorbar")
    sub.set_xticklabels([''] + input_sentence.split(' ') + ['<EOS>'], rotation=90)
    sub.set_yticklabels([''] + output_words)
    #print("added labels")
    sub.xaxis.set_major_locator(ticker.MultipleLocator(1))
    sub.yaxis.set_major_locator(ticker.MultipleLocator(1))
    #print("added multiple labels")
    plt.show()
...
if __name__ == '__main__':
    ...
    trainIters(encoder1, decoder1, 35000, print_every=5000,
               use_attention=True)
    
    input_sentence = "tu es probablement trop jeune pour le comprendre ."
    output_words, attentions = evaluate(encoder1, decoder1, input_sentence)

    savepath = "attention.png"
    diagramMaker(output_words, attentions, input_sentence, savepath)
