...
def softmax(x):
  ...
  ### YOUR CODE HERE
  maxes = tf.reduce_max(x, axis=1, keep_dims=True)
  sub = x - maxes
  exp = tf.exp(sub)
  sum = tf.reduce_sum(exp, axis=1, keep_dims=True)
  out = tf.divide(exp, sum)
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  """
  ### YOUR CODE HERE
  yhatlog = tf.log(yhat) # on google colab, this is float64 and must be cast to 32
  yfloat = tf.to_float(y)
  multiply = tf.multiply(yhatlog, yfloat) # only then can you multiply
  out = - tf.reduce_sum(multiply)
  ### END YOUR CODE
  return out
